...
<!-- 본문내용 -->
	...
	<div class="top">
		...
		%>
		<div class="write">
			<button type="button" onclick="location.href='./boardWriteAction.bo'">글쓰기</button>
		</div>
		<% } %>
		<!-- 검색  -->
		<div class="search">	
	...
	for (int i = 0 ; i<boardList.size() ; i++) {
	 ...
	 <tr onclick="location.href='./boardContentAction.bo?num=<%=bean.getNum() %>&pageNum=<%=pageNum%>'">
	  	...
	  	<td class="tt"><%=bean.getSubject() %></td>
	  	<td class="tw"><%=bean.getId()%></td>
	  	<td class="td"><%=bean.getDate()%></td>
	...
	<div id="page">
		...
		%>
			<a href="./boardSearchList.bo?pageNum=<%=startPage-pageBlock%>&search=<%=search%>"> < </a>
		<%
			}
			for(int i=startPage;i<=endPage;i++){
		%>
			<a href="./boardSearchList.bo?pageNum=<%=i%>&search=<%=search%>"><%=i %></a>
		<%
			}
